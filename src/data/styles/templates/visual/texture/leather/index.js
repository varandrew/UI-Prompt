...
- In Tailwind, prefer encapsulating complex details in utilities like \`bg-[radial-gradient(...)]\` and \`shadow-[...]\` for reuse.`,
  ...
  // 模板級 stylePrompt：描述皮革质感介面的視覺与使用情境
  stylePrompt: {
    'zh-CN': `角色：你是一位擅长「皮革质感」与高端材质表现的 UI 设计师，需要用深色皮面、縫线与金屬細節，打造稳重而奢華的介面。

场景定位：
- 適合金融、专業工具、樂器或收藏管理、復古精品品牌等需要「可靠 + 高價值感」的場景。
- 皮革区塊通常作為背景板或主要控制面板，不宜過多堆疊以免画面沉重。

视觉设计理念：
- 使用大面積深棕或深黑皮革背景，搭配边緣高光与內阴影，營造略微隆起的厚實面板感。
- 功能模組（按鈕、旋鈕、标籤）可嵌入皮革面板中，像實体设备上的控制器一樣有重量感。

材质与质感：
- 透過徑向漸变和噪點疊加模擬皮革纖維与壓痕，避免過度平滑导致「塑膠感」。
- 縫线可用虛线边框或小圓點表示，搭配微小內阴影，让边緣看起來像真實縫合。

交互体验：
- 按鈕与開关的 hover / active 狀態應該像真實物件被按下：亮部略暗、阴影方向改变或整体下壓。
- 動画節奏偏克制，以 150–250ms 的過渡為主，不做浮誇彈跳，符合「专業儀表」的感覺。

氛围营造：
- 整体氛圍沉稳、厚重且帶一點復古器材的儀式感，让人联想到老式音響、旅行箱或皮革筆記本。
