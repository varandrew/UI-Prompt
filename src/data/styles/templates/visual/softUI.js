...
- Transition 设计：
  ...
  - 禁止使用巨大旋转、弹簧动画等破坏 Soft UI 气質的動效。

【排版與圖標】
- 字體選擇：
  - 使用現代無襯線字體作為基礎，例如系統預設或 Tailwind 默認（Inter 類），透過不同字重營造層次。
  - 標題（如 h1/h2）字重可用 700–800，文字顏色用深灰（#111827 / #0f172a），正文用中灰（#4b5563 / #6b7280）。
- 圖標風格：
  - 圖標本身保持線性或半填充樣式，配合柔和漸變或 Soft UI icon 容器（圓形/圓角矩形背景 + 雙向陰影）。

【輸出技術要求】
- 使用完整 HTML 結構（<header>、<nav>、<main>、<section>、<aside>、<footer>），並用 TailwindCSS 風格原子類控制佈局和間距（如 flex、grid、gap-6、px-6、py-4、rounded-3xl、shadow-*、bg-slate-*、text-slate-*）。
- 不輸出解說文字或註釋，只輸出最終 HTML。
- 結構上必須包含：頂部導航、Hero 區、統計卡片網格、主內容 + 側欄、（可選）底部區域。
- 視覺上必須清晰傳達 Soft UI / Neumorphism 特色：柔和圓角、內外陰影、低飽和度背景、細膩光影，而不是單純平面扁平化卡片。
`,
    'en-US': `
...
材质与质感：
背景通常是非常浅的灰蓝、灰白或暖灰，带有很轻的渐变或颗粒，营造「细腻的塑料/橡膠」質感。卡片不應該看起來像貼在背景上，而是像從背景表面「鼓起」來；開關、按鈕或輸入框則像被「壓」進去。外陰影與內陰影同時存在：一側是柔和的深色陰影，另一側是細膩的高光，讓元素既有厚度又不顯沉重。

排版與佈局：
在 Soft UI 中，排版依然要遵守現代介面的通用原則：清晰的標題層級、舒適的行距、充足的留白。不同的是，邊界感更多來自光影而不是銳利的線條。你會減少使用實線分隔，改用區塊之間的間距、陰影差異、背景明暗變化來分組。例如，一組相關卡片可以共用一塊略深的背景區域，再在其上做淺浮雕式的卡片，製造次級層次。

交互體驗：
Soft UI 的交互應該像在觸摸柔軟材質：懸停時元素輕微抬起或變亮，按下時則微微內凹、陰影縮小。這些變化幅度要小而精準，讓使用者感受到回饋，但不會被誇張動畫打斷思路。焦點狀態可以透過內陰影和高光邊框來表現，而不是把邊框變成鮮豔的實線。整體節奏是順滑的、漸進的，而不是硬切和閃動。

氛圍營造：
好的 Soft UI 讓人聯想到現代作業系統的設定面板或高級家電的觸控介面：柔和、安靜、具備高質感。界面看起來「有溫度」但不幼稚，「有科技感」但不冷酷。長時間停留在這樣的界面裡，不會因為高對比色彩或尖銳邊緣而感到疲勞。所有設計決策——色彩、陰影、圓角、字體——都應圍繞這種舒適、穩定的氛圍展開。
`,
    'en-US': `
Role:
