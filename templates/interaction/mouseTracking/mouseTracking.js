...
4. 交互行为（Mouse Tracking 交互細節）
   ...
   - 页面文案应明确提示用戶「Move your mouse」「Hover to explore」，使交互意圖清晰。

5. 粒子尾迹效果（Cursor Particle Trail）
   - 在页面全局监听鼠标移动事件，生成带有寿命与透明度递减的「粒子」：
     - 每个粒子包含：初始位置（接近光标）、随机速度向量、大小、颜色（可以用 HSL 在藍色与紫色区間內变化）、不透明度。
     - 動画步驟中更新位置、縮小大小、降低透明度，當粒子過小或完全透明時移除。
...
- Make clear in the overall Prompt that the new layout must feel unmistakably like a variation of the existing Mouse Tracking Interaction page: same family of colors, same 3D parallax behavior, same style of coordinate readouts and glowing cursor trail, but with fresh copy and slightly different composition.`
  ...
  stylePrompt: {
    'zh-CN': `只讨论「鼠标追踪」：以光标为中心的感应区，卡片或光晕根据鼠标位置做小幅位移、倾斜或亮度变化，突出指针随动的存在感。保持反馈克制但连续，使用 transform 等硬件加速属性确保流畅，不掺杂手势或语音等其他交互。适合用于登陆页 Hero、控制面板或交互展示，让用户一移动鼠标就能看到界面响应。`,
    'en-US': `Mouse-tracking only: build a cursor-aware surface where cards or glows shift subtly with pointer position to emphasize follow behavior. Keep responses continuous yet restrained, use transform-based, hardware-accelerated animations for smoothness, and avoid unrelated gesture/voice interactions. Great for hero sections, control panels, or interaction showcases where moving the mouse immediately proves the UI is responsive.`
  }
};
