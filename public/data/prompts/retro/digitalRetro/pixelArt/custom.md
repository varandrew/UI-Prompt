...
3. **像素按钮与卡片**
   ...
   - 示例：
     ...
     .pixel-btn {
       ...
       box-shadow: 0 3px #1b1b1b, 3px 0 #1b1b1b, 0 -3px #1b1b1b, -3px 0 #1b1b1b;
...
4. **字体与排版**
   - 标题/按钮：像素或等宽字体；正文可用清晰无衬线；行距 1.4–1.6。

5. **布局**
   - Hero（大号像素标题 + CTA 双按钮），下方网格卡片（关卡/功能/案例），右侧或底部放排行榜/提示框；保持模块化。

6. **动效**
   - Hover 仅颜色反转或微小平移（2–3px）；禁用模糊阴影，用硬边阴影。
...
```css
.pixel-shell { padding: 24px 20px; background: #0f1021; }
.pixel-grid { display: grid; gap: 16px; }
.pixel-card { background: #1f2540; border: 3px solid #1b1b1b; box-shadow: 0 4px #1b1b1b; padding: 16px; border-radius: 0; }
.pixel-title { font-family: 'Press Start 2P', monospace; letter-spacing: 0.04em; }
.pixel-btn { display: inline-flex; align-items: center; justify-content: center; padding: 12px 18px; }
.pixel-alert { background: #ff5e5b; color: #1b1b1b; border: 3px solid #1b1b1b; }
```

**可选部分：间距 / 图标**
- 间距基线 8px；卡片内边距 16px；区块间距 20–24px。
- 图标使用 16px 像素图或纯文本符号；避免矢量柔光。

**重要提示**
- 保留 `.pixel-shell`、`.pixel-card`、`.pixel-btn`、`.pixel-grid` 等类名；禁止模糊阴影与圆角。
- 背景动效尽量避免渐变模糊，以纯色块或 tile 呈现。
- 输出完整 HTML（header/main/section/footer），不含额外解释。
